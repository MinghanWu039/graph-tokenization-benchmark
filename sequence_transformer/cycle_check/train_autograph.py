...
try:
	from .dataloader import AutographDataset, split_dataset
...
def compute_vocab_size(dataset: Dataset) -> int:
	max_token = 0
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--pad-token-id", type=int, default=0, help="Token id used for padding sequences.")
...
def main() -> None:
	...
	collate_fn = make_collate_fn(args.pad_token_id, args.max_len)
	train_loader = DataLoader(train_subset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn)
	val_loader = (
		DataLoader(val_subset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn) if val_subset else None
	)
	test_loader = (
		DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn) if test_dataset else None
	)
